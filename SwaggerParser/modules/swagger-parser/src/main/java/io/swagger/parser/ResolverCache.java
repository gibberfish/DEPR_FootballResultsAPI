...
public class ResolverCache {
    ...
    public <T> T loadRef(String ref, RefFormat refFormat, Class<T> expectedType) {
        ...
        Object previouslyResolvedEntity = getEntityFromCache(ref);
    ...
    private Object getEntityFromCache(String ref) {
        Object entity = resolutionCache.get(ref);
        if (entity == null) {
            entity = getEntityByPartialName(ref);
        }

        return entity;
    }

    private Object getEntityByPartialName(String ref) {
        String[] pieces = ref.split("#/");
        if (pieces.length != 2) {
            return null;
        }
        String definitionPath = pieces[1];
        for (Map.Entry<String, Object> entry : this.resolutionCache.entrySet()) {
            String key = entry.getKey();

            if (key.endsWith(definitionPath)) {
